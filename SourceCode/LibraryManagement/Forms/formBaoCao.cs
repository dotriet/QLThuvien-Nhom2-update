...
namespace DemoDesign
{
    ...
    {
        List<BaoCaoTheoLoai> reportByCategory;
        List<BaoCaoTraTre> reportLate;
        ...
        {
            reportByCategory = new List<BaoCaoTheoLoai>();
            reportLate = new List<BaoCaoTraTre>();
        ...
        private void CategoryReport()
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    reportByCategory.Add(new BaoCaoTheoLoai(reader.GetString(0), reader.GetInt32(1)));
                }
            }
            int stt = 1;
            float totalNumOfBorrowings = 0;
            foreach(BaoCaoTheoLoai report in reportByCategory)
            {
                report.stt = stt;
        ...
        private void LateReport()
        {
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                ...
                while (reader.Read())
                {
                    reportLate.Add(new BaoCaoTraTre(reader.GetString(0), reader.GetString(1), reader.GetDateTime(2).ToString("dd/MM/yyyy")));

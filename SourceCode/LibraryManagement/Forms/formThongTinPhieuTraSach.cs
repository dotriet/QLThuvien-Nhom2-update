...
{
    public partial class formThongTinPhieuTraSach : Form
    {
        BindingSource bindingChosen;

        public static DSPhieuTra returnSlip;
        ...
        public formThongTinPhieuTraSach()
        {
            InitializeComponent();
        ...
        private void UpdataData()
        {
            ...
            foreach (TraSach book in returnSlip.returnBooks)
            {
                //foreach()
                createReturnSlipDetail += $@"INSERT INTO CTPT(MaPhieuTraSach, MaCuonSach, MaPhieuMuonSach, SoNgayMuon, TienPhat) VALUES('{returnSlip.recvSlipCode}','{book.specBookCode}','MPMS010','{book.borrowedDays}','{book.fine}')" + "\n";
                setBookAndSlipDetailStatus += $@"UPDATE CTPHIEUMUON SET TinhTrangPM = 1  WHERE MaCuonSach = '{book.specBookCode}'" + "\n" + $@"UPDATE CUONSACH SET TinhTrang = 0 WHERE MaCuonSach = '{book.specBookCode}'";
            }
            ...
            DemoDesign.formNhanTraSach.recvState = "Success";
            //SendMail();
            this.Close();
        }
        ...
        {
            DemoDesign.formNhanTraSach.recvState = "Cancelled";
            this.Close();
        }
